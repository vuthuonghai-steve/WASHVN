...
## 1a. 7 LLM Principles Mapping

> Each Golden Standard maps to one of the 7 LLM Operating Principles.

| Golden Standard | LLM Principle | Rationale |
|----------------|---------------|-----------|
| **Reusability** | (1) Domain Anchoring | Reusable skills anchor to domain knowledge files, not hardcoded logic |
| **Composability** | (2) Semantic over Ceremony | Contract-driven composition via `.skill-context/` favors semantic fit over rigid interfaces |
| **Maintainability** | (3) Context Pre-processing | Goldilocks zone / 4-layer structure pre-processes context into optimal token tiers |
| **Security** | (6) Negative Space | Security boundaries (XML tags, sandbox) define what the system MUST NOT do |
| **Context Efficiency** | (4) Dual Knowledge Stream | Progressive Disclosure separates boot (Tier 1) vs domain (Tier 2) vs examples (Tier 3) |
| **Portability** | (2) Semantic over Ceremony | Model-agnostic prompts favor semantic clarity over API-specific ceremony |
| **Reliability & Fallback** | (7) Graceful Degradation | Fallback plans + HITL ensure controlled failure instead of hallucination |

---

## 1b. SCS 2-Phase Validation Protocol

> Dual-pass complexity validation to balance rigor vs cost.

- **Phase SCS-1 (Stage 0.5 — Knowledge Miner, mandatory for v1.0)**: Pre-pass. Compute SCS from available metadata (estimated process steps, tools, token size, security risk). If SCS ≤ 3.0 AND no red-flag (5-point) dimension, pass directly — no decomposition overhead for simple skills.
- **Phase SCS-2 (Stage 1.5 — Gatekeeper, optional for v1.0)**: Full validation. Re-run SCS with complete design artifacts. Cross-check against Stage-0.5 result. Flag drift (SCS delta > 1.0). This becomes mandatory in v2.0.
