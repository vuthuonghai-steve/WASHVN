#!/usr/bin/env python3
"""
init_context.py — Khởi tạo cấu trúc thư mục .skill-context/{target_skill}/ cho Stage 0: skill-explorer.
...
def handle_single_init(skill_name: str, project_root: Path, script_dir: Path) -> int:
    ...
    # Also create hydrated-context.yaml and thought-cache.yaml from templates
    for artifact_name in ("hydrated-context.yaml", "thought-cache.yaml"):
        template_file = script_dir.parent / "templates" / artifact_name
        target_file = skill_context_dir / artifact_name
        if template_file.is_file():
            raw_content = template_file.read_text(encoding="utf-8")
            content = replace_placeholders(raw_content, replacements)
            status = safe_create_file(target_file, content)
            print(f"  {artifact_name:<20} → {status}")
        else:
            fallback = f"# {skill_name}\n"
            status = safe_create_file(target_file, fallback)
            print(f"  {artifact_name:<20} → {status}")

    print(f"\nDone: {resources_dir}, exploration.md, hydrated-context.yaml, and thought-cache.yaml configured successfully.")
    return 0
...
def handle_split_run(exploration_path: Path, project_root: Path) -> int:
    ...
    for item in micro_skills:
        ...
        # Also copy hydrated-context.yaml and thought-cache.yaml from master context dir
        master_dir = exploration_path.parent
        for art in ("hydrated-context.yaml", "thought-cache.yaml"):
            src = master_dir / art
            if src.is_file():
                shutil.copy(src, child_context_dir / art)
                copied_resources.append(art)
