...
| 3 | Agent Foundation Build | P0, P1, P2 | M-L | 9 | 🟡 in_progress | ~70% (9/9 agents trên disk, chưa commit, thiếu aggregate-quality-gatekeeper) | 2026-07-08 | — |
...
- [ ] **Task 1**: Invoke subagent-forge to design `skill-pipeline-orchestrator` → commit `phase-3: skill-pipeline-orchestrator agent staged`
- [ ] **Task 2**: Review subagent-forge output — verify APPROVED_FOR_REVIEW, fix NEEDS_FIX
- [ ] **Task 3**: Deploy orchestrator agent → commit `phase-3: orchestrator agent deployed`
- [ ] **Task 4**: Invoke subagent-forge for `aggregate-quality-gatekeeper` → commit `phase-3: aggregate-quality-gatekeeper agent staged`
- [ ] **Task 5**: Review + deploy aggregate-quality-gatekeeper → commit `phase-3: gatekeeper agent deployed`
- [ ] **Task 6**: Invoke subagent-forge for `ba-pipeline-runner` → commit `phase-3: ba-pipeline-runner agent staged`
- [ ] **Task 7**: Review + deploy ba-pipeline-runner → commit `phase-3: ba-pipeline runner agent deployed`
- [ ] **Task 8**: Invoke subagent-forge for `external-code-reviewer` → commit `phase-3: external-code-reviewer agent staged`
- [ ] **Task 9**: Review + deploy external-code-reviewer → commit `phase-3: external reviewer agent deployed`
- [ ] **Task 10**: Run full AC-1 to AC-8, fix any failures
- [ ] **Task 11**: Update `workspce_tree.md` với 4 new agent entries
- [ ] **Task 12**: Author Phase 3 summary doc `docs/context-to-work/foundation-bootstrap/phase-3-summary.2026-07-04.md`

### Deliverables (4 agents)
- [ ] `skill-pipeline-orchestrator.md` — Orchestrate 8-stage pipeline, block recursion (max depth=1), PreToolUse hooks gate writes
- [ ] `aggregate-quality-gatekeeper.md` — External validator (Γ-1 fix), sonnet model, chỉ write `.skill-context/`
- [ ] `ba-pipeline-runner.md` — Orchestrate 3 BA skills, chỉ write `.skill-context/{feature}/ba-*`
- [ ] `external-code-reviewer.md` — Fresh-eyes reviewer (Γ-1 fix), sonnet, block code execution (static analysis only)

### Acceptance Criteria (8)

- [ ] **AC-1: 4 agent files tại staging** — `test -f .claude/agents/_staging/<name>.md`
- [ ] **AC-2: Frontmatter YAML parses** — name, description, model, tools, permissionMode + không bypassPermissions
- [ ] **AC-3: Reference 7 knowledge docs** — grep count ≥ 7 per agent
- [ ] **AC-4: Inline hooks block** — grep "exit 2" per agent
- [ ] **AC-5: Subagent-forge 4-evaluator** — invoke per agent, verify APPROVED_FOR_REVIEW (MANUAL)
- [ ] **AC-6: Output contract section** — grep `<output_contract>` per agent
- [ ] **AC-7: Skills referenced exist** — Phase 5/6 build (WARNING OK tại Phase 3)
- [ ] **AC-8: No bypassPermissions** — grep fail if found

### Definition of Done

- [ ] 4 agents deployed tại `.claude/agents/` runtime
- [ ] All 4 agents PASS subagent-forge 4-evaluator ≥ APPROVED_FOR_REVIEW
...
- [ ] Each agent có `<output_contract>` section với concrete artifact paths
